...
load("MK_Full_Room1_r4.mat");

% x = Mesh.Nodes(Mesh.Connectivity(1,2:5),1); x(5) = x(1);
% y = Mesh.Nodes(Mesh.Connectivity(1,2:5),2); y(5) = y(1);
% figure,plot(x,y),hold on;
% plot(Mesh.Nodes(Mesh.Connectivity(1,2:end),1), ...
%     Mesh.Nodes(Mesh.Connectivity(1,2:end),2),"ro")
...
U0 = a*b.*exp(-d./2./(Lambda^2));
            % triii = delaunay(xnd,ynd);
            % figure, trisurf(triii, xnd, ynd, abs(fftshift(fft(U0))))
% Nt_L = 301*5;
...
figure,
tmm = dt_M.*chk;
% dt_plt = zeros(length(dT_L),length(m_L)+1);
dt_plt = [tmm tmm(:,end)];

surf([m_L 11],dT_L,dt_plt)
...
figure(5),
plot(m_L,c.*plt_dt./h,"*-",LineWidth=1.4,MarkerSize=10);
ylim([0 1]);
disp("max delta t:")
disp(2/max(eigs(-M,K,6,'largestabs')))
grid on
xlabel("order"), ylabel("CFL");
fontsize(figure(5), 16, "points")

%% Boundary condititions

function plotting(Mesh,plt_lines,plt_nodes,txt_plt)
if plt_lines
figure;
hold on;
axis equal;
