        ...
        for in2 = 1 : size(N,1)
            for in1 = 1 : size(N,1)
                ...
                count = count + 1 ;
                    ...
                    ktemp = zeros((Mesh.Order+1)^2, (Mesh.Order+1)^2,size(Mesh.Connectivity,1));
                    mtemp = zeros((Mesh.Order+1)^2, (Mesh.Order+1)^2,size(Mesh.Connectivity,1));
...
for iel = 1:size(Mesh.Connectivity,1)
    ...
    for iwy = 1:numel(x)
        for iwx = 1:numel(x)
            ...
            ktemp(:,:,iel) = ktemp(:,:,iel) + w(iwx)*w(iwy)*det(J)*(B'*matC*B);
            NN = squeeze(phi(iwx,iwy,:))';              
            mtemp(:,:,iel) = mtemp(:,:,iel) + w(iwx)*w(iwy)*det(J)*(NN'*matRho*NN);
        end
    end
    
    % Assembly into global matrices
    K = AssemblyGlobal(K, ktemp(:,:,iel), nodes);  % Stiffness matrix
    M = AssemblyGlobal(M, mtemp(:,:,iel), nodes);  % Mass matrix 
end
end
