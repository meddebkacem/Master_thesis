...
sep_id = find(dd<=0.3);    % Ref to get just the surrounding elements
% for ki=size(sep_id):-1:1
%     if (any(Mesh.Nodes(Mesh.Connectivity(sep_id(ki),2:5),1)<=4.68))||(any(Mesh.Nodes(Mesh.Connectivity(sep_id(ki),2:5),1)>=5.5))...
%         ||(any(Mesh.Nodes(Mesh.Connectivity(sep_id(ki),2:5),2)<=1.79))||(any(Mesh.Nodes(Mesh.Connectivity(sep_id(ki),2:5),2)>=2.42))
%         sep_id(ki) = [];
%     end
% end

%%
% class_of_elts = zeros(10,1);
% kk = 0.2:0.1:1.3;
% for kkl=1:12
