...
Nel_L = [10:10:100 120:20:200 250:50:2000];
% Nel_L = 400;
h_L = 2*L./Nel_L;
...
% figure(1),surf(X,((1:1:N_t)./N_t)*TM,UUU'),shading flat
% xlabel("Space domain"),ylabel("time")
% 
% figure(2),
% subplot(4,1,1),plot(X,UUU(:,1),LineWidth=1.5), 
% xlabel("Space domain"),ylabel("Amplitude"),grid on,ylim([-0.1 1.1]),
% subplot(4,1,2),plot(X,UUU(:,200),LineWidth=1.5), 
% xlabel("Space domain"),ylabel("Amplitude"),grid on,ylim([-0.1 1.1]),
% subplot(4,1,3),plot(X,UUU(:,400),LineWidth=1.5), 
% xlabel("Space domain"),ylabel("Amplitude"),grid on,ylim([-0.1 1.1]),
% subplot(4,1,4),plot(X,UUU(:,600),LineWidth=1.5), 
% xlabel("Space domain"),ylabel("Amplitude"),grid on,ylim([-0.1 1.1]),
% sgtitle("Wave propagation in different time points")
...
EV = EV + 1;
% figure()
% surf(dT_L,h_L,EV.*CFL), shading flat;
% xlabel("\Delta t"), ylabel("\Delta x"), zlabel("CFL value"), % zscale log;
