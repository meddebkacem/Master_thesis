...
%%%%%%%%%% Related
% last paragraph 241/355 PDF
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% test for fixed Δt = 0.005
% alp = 0.01:0.01:0.8;
% dTList =  0.005;
% hList = c*dTList./alp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% test for fixed h = 0.02  ~~~~
% dTList =  0.00125:0.00125:0.04;
% for ik=1:length(dTList)
%     if mod(2*TM,dTList(ik))~=0
%         dTList(ik)=0;
%     end
% end
% dTList = nonzeros(dTList);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% test for fixed h = 0.01  ~~~~
% hList = 0.01;
% dTList =  0.0002:0.0002:0.02;
% for ik=1:length(dTList)
%     if mod(TM,dTList(ik))
%         dTList(ik)=0;
%     end
...
% hList = 0.02;                           % for Δt:cte
% alp = 0.01:0.005:1;
% dTList = alp*hList;

alp = 0.001:0.001:0.1;              % for Δx:cte
dTList =  0.001;
hList = (c*dTList./alp);
...
figure()
loglog(hList,err_M,"LineWidth",1),
xlabel("\Delta x"),ylabel("Error");

figure()
loglog(round(2*L./hList),err_M,"LineWidth",1),
xlabel("Nb of elts"),ylabel("Error");
