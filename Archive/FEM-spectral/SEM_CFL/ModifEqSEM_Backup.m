...
c = 1; L = 1; TM = 0.6;
Lambda = L/10;
rl = 1:1:5;                                  % order of interp r-1
ml = 1:1:20;                                 % order of taylor approx

dT =  0.005;
h = 0.05;

err_M = zeros(length(rl),length(ml));
...
for jr=1:length(rl)
    ...
    parfor jm=1:length(ml)
        ...
        % plot(X,Unp);hold on,plot(X,theo_fn,"--","LineWidth",1);
        % ylim([-0.1 1.1]);xlabel("Position (m)");ylabel("amplitude");grid on;
...
figure(),
plot(ml,err_M),legend("r=1","r=2","r=3","r=4","r=5")
